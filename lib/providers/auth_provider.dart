...
class AuthProvider with ChangeNotifier {
  ...
  Future<void> loginAsAdmin() async {
    try {
      // On Web, Firestore rules usually require request.auth != null.
      // Signing in anonymously satisfies this requirement.
      await _auth.signInAnonymously();

      _user = User(
        id: 'admin_123',
        role: UserRole.admin,
        fullName: 'مدير النظام (Mock)',
        phone: '01000000000',
        nationalIdHash: 'admin_hash',
        address: 'المقر الرئيسي',
        createdAt: DateTime.now(),
      );
      debugPrint('AUTH_PROVIDER: Logged in as Mock Admin (Anonymous Session).');
      notifyListeners();
    } catch (e) {
      debugPrint('AUTH_PROVIDER: Anonymous Sign-in failed: $e');
      // Fallback to mock user even if anonymous sign-in fails
      _user = User(
        id: 'admin_123',
        role: UserRole.admin,
        fullName: 'مدير النظام (Mock)',
