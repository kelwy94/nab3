...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  _SellerPaymentInfo _extractPaymentInfo(Map<String, dynamic>? sellerData) {
    if (sellerData == null)
      return _SellerPaymentInfo(
          'غير محدد', '', sellerData?['fullName'] ?? 'البائع');
    ...
    final methodStr = sellerPaymentMethod?.toString() ??
        extraData?['paymentMethod']?.toString() ??
        '';
    if (methodStr.contains('vodafoneCash')) {
      methodLabel = 'فودافون كاش';
    } else if (methodStr.contains('instaPay'))
  ...
  void _startCheckout() async {
    ...
    for (int i = 0; i < sellerIds.length; i++) {
      ...
      double sellerSubtotal = sellerItems.fold(0.0,
          (sum, item) => sum + (item.price * appState.getQuantity(item.id)));
      ...
      if (payInfo.methodLabel == 'غير محدد' || payInfo.details.isEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                  'البائع "${payInfo.name}" لم يسجل بيانات الدفع. تواصل معه مباشرة.'),
  ...
  Widget build(BuildContext context) {
    ...
    final subtotal = cart.fold(
        0.0, (sum, item) => sum + (item.price * appState.getQuantity(item.id)));
    ...
    final itemCount =
        cart.fold(0, (sum, item) => sum + appState.getQuantity(item.id));
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(selectedAddress ?? 'لم يتم تحديد العنوان',
            ...
            child: InkWell(
              ...
              child: Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 12),
                  const Expanded(
                      child: Text('اترك عند الباب',
                          style: TextStyle(fontSize: 14),
                          textAlign: TextAlign.right)),
                  Checkbox(
                      value: _leaveAtDoor,
                      onChanged: (v) =>
                          setState(() => _leaveAtDoor = v ?? false),
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                _summaryRow(
                    'رسوم التوصيل', '${deliveryFee.toStringAsFixed(2)} ج.م',
                ...
                Row(
                  ...
                  children: [
                    const Text('الإجمالي شامل الضريبة',
                        style: TextStyle(
                ...
                ListTile(
                  ...
                  title: const Text('إضافة عنوان جديد',
                      ...
                      style: TextStyle(
                          ...
                          fontWeight: FontWeight.bold)),
                  trailing:
                      const Icon(Icons.chevron_left, color: NabaTheme.primary),
              ...
              ...addresses.map((addr) {
                ...
                return ListTile(
                  leading: Icon(
                      ...
                      color: isSel ? NabaTheme.primary : Colors.grey),
                  title: Text(addr,
                      textAlign: TextAlign.right,
                      style: TextStyle(
                          fontWeight:
